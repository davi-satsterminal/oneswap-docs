...
The quote response also includes a top-level `settlementSafety` field — `null` when the swap is clear to proceed, or `{ code, reason, message }` when a swap of this size/timing would be blocked at settlement. Inspect it before calling `swaps.create`; see [Settlement safety](#settlement-safety).

### Fee discounts

If a fee discount applies — from the SDK API key, the wallet party, or the pool (see [Fee discounts](/reference/types#fee-discounts)) — the charged amount fields already reflect it, and the applied policy is returned in `networkFeePolicy` / `platformFeePolicy` (otherwise `null`). The same quote without and with a pool discount (50% off network, 30% off platform):

<CodeGroup>
```json No discount
{
  "outputAmount": "0.0000277700",
  "fee": 0.003,
  "networkFeeAmount": "7.2871950000",
  "platformFeeAmount": "0.0132846037",
  "networkFeePolicy": null,
...
```json With discount
{
  "outputAmount": "0.0000334890",            // +20.6% more output
  "fee": 0.003,                              // nominal rate — unchanged
  "networkFeeAmount": "3.6435975000",        // 50% off
  "platformFeeAmount": "0.0112121113",
  "networkFeePolicy": {
    "targetType": "pool",
    "chargeBps": 5000,
    "estimatedNetworkFeeAmount": "7.2871950000",
    "chargedNetworkFeeAmount": "3.6435975000"
  },
  "platformFeePolicy": {
    "targetType": "pool",
    "chargeBps": 7000,
    "fullPlatformFeeRate": "0.0007500000",
    "chargedPlatformFeeRate": "0.0005250000",
    "discountBps": 3000
  }
}
```
...
The top-level `fee` rate stays nominal regardless of the discount — read the charged amount fields or `discountBps`. A wallet discount only resolves in a quote when you pass `receiverParty`.
...
Check it before calling `swaps.create`. If you skip the check and the guard would block, `swaps.create` returns HTTP `400` and the SDK throws [`SettlementSafetyError`](/guides/error-handling#settlement-safety-guard) (which carries the same `code`/`reason`/`message`); no intent is created and no deposit happens.
