...
```typescript
const positions = await client.positions.list('alice')
...
```typescript
const earnings = await client.positions.getEarnings('alice', 'Amulet-USDCx')

console.log(`APR: ${earnings.apr}%`)
console.log(`Daily: ${earnings.estimatedDailyEarnings.tokenA} Amulet`)
console.log(`Daily: ${earnings.estimatedDailyEarnings.tokenB} USDCx`)
```
