...
import {
  ...
  PriceImpactError, // swap rejected because price impact exceeded the server cap
...
| `PriceImpactError` | Swap would move the price beyond the server-enforced cap (default 15%) | Reduce swap size or add liquidity to the pool |
