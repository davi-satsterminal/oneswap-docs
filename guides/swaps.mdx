...
| `depositReference` | Transfer reference to include in the Canton reason/reference field |
...
6. The user sends the input token directly from that same party to `depositAddress` and includes the reference in the Canton reason/reference field
...
OneSwap enforces a server-side price impact cap (currently 15%). If a swap would move the pool price beyond this limit, the quote endpoint returns a `400` error and any deposited funds are refunded automatically with status `price_impact_exceeded`. The cap is returned in `quote.maxPriceImpact`. To execute large swaps, either split them into smaller amounts or add liquidity to the pool first.
...
`depositReference` is part of the live swap contract. Treat it as required operational data and pass it through to the wallet UX that sends the Canton transfer.
