...
<Tip>
  Use [`quotes.get`](/reference/sdk-methods) to price the swap first — its
  `amountOut` is the natural basis for a sensible `minOut` (leave room for the
  network fee, which is settled from the input).
</Tip>
